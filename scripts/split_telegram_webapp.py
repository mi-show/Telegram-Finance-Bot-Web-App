#!/usr/bin/env python3
"""Split Telegram WebApp vendor source into chunks and regenerate bootstrap file.

Example:
  python scripts/split_telegram_webapp.py \
    --source third_party/telegram-webapp.js \
...
class SplitConfig:
  ...
  target_dir: Path
  chunk_size: int
  chunk_folder_name: str = "webapp_chunks"
  bootstrap_filename: str = "webapp.js"
  ...
  def _write_chunks(self, chunks: list[str]) -> None:
    ...
    for index, chunk in enumerate(chunks, start=1):
      ...
      chunk_file.write_text(
        "window.__TelegramWebAppChunkStore = window.__TelegramWebAppChunkStore || [];\n"
        + f"window.__TelegramWebAppChunkStore.push({json.dumps(chunk)});\n",
        encoding="utf-8",
      )
...
def build_parser() -> argparse.ArgumentParser:
  parser = argparse.ArgumentParser(
    description="Split Telegram WebApp vendor source into chunks and bootstrap loader."
  )
  parser.add_argument(
    "--source",
    required=True,
    type=Path,
    help="Path to original Telegram WebApp source (single file).",
  )
  parser.add_argument(
    "--target-dir",
    default=Path("app/web/static/vendor/telegram"),
    type=Path,
    help="Directory where webapp_chunks and webapp.js bootstrap will be written.",
  )
  parser.add_argument(
    "--chunk-size",
    default=45000,
...
def main() -> int:
  ...
  splitter = TelegramWebAppSplitter(config)
  splitter.run()
  return 0
